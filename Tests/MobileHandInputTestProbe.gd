...
func _run() -> void:
	...
	assert(gestures.drop_label.visible and gestures.drop_label.get_parent() == gestures, "Drop hint must be independent of the target area")
	assert(gestures.drop_label.get_global_rect().end.y <= scene.action_label.global_position.y, "Drop hint must stay above the turn text")
	assert(gestures.drop_label.global_position.y >= target.y, "Drop hint must stay in the lower half of the table")
	assert(not scene.mobile_premove_hint.visible, "Dragging must not show two overlapping hints")
	_touch(Vector2(100, 900), false)
	assert(confirmations.size() == count and a.modulate.a == 1.0, "Drop outside returns the card")
	assert(not gestures.drop_label.visible, "Drop hint must disappear on cancellation")
	_touch(point, true)
	_drag(target)
	_touch(target, false)
	...
	j = _view(joker)
	_tap(j)
	assert(scene.pending_joker_card == joker, "First Joker tap opens the condition picker")
	...
	# Another bot plays while the human is choosing a premove.
	var next_bot_card: Card = scene._create_card(Card.Suit.CLUBS, Card.Rank.KING, false)
	scene.game.players[2].hand.append(next_bot_card)
	assert(scene.game.active_trick.play_card(scene.game.players[2], next_bot_card))
	scene._refresh_hand()
	await process_frame
	await process_frame
	a = _view(clubs)
	assert(a.is_selected, "Selection must survive a move between opponents")
	_tap(a)
