...
func _run() -> void:
	...
	scene._on_reaction_toggle_pressed()
	await _settle()
	assert(scene.reaction_picker.visible, "Reaction picker must open while a bid is pending")
	assert(scene.mobile_bid_popup.visible, "Opening reactions must not hide the pending bid choices")
	scene.reaction_picker.hide()
