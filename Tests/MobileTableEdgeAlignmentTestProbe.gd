...
func _run() -> void:
	...
	for locale in ["ru", "en", "uk", "pl", "be", "kz"]:
		...
		assert(not scene.trump_label.visible, "Trump is shown at the deck, not repeated in the header")
		...
		assert(history_rect.position.x - bar_rect.position.x >= scene.PhoneTable.SAFE_LEFT - 0.5, "History must clear the left phone cutout")
		assert(is_equal_approx(bar_rect.end.x - menu_rect.end.x, 0.0), "Menu must sit at the right inner edge")
		...
		for button in [scene.round_history_toggle_button, scene.score_sheet_toggle_button, scene.pause_menu_button]:
			...
			for state in ["normal", "hover", "pressed", "disabled"]:
				var style: StyleBoxTexture = button.get_theme_stylebox(state)
				assert(style.content_margin_left == 18.0 and style.content_margin_right == 18.0, "All top actions must have identical balanced padding")
...
func _check_card_gaps(scene: Variant, count: int) -> void:
	for slot in [1, 2, 3]:
		...
		assert(is_equal_approx(gap, 8.0), "Slot %d with %d cards must retain the same eight-pixel gap, got %s" % [slot, count, gap])