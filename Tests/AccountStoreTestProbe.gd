...
func _init() -> void:
	...
	var starter_inventory: Dictionary = account.get("inventory", {})
	assert((starter_inventory.get("card_backs", []) as Array).size() == 1)
	assert((starter_inventory.get("avatars", []) as Array).size() == 4)
	...
	assert((xp_grant.get("newly_unlocked", []) as Array).is_empty())
	var threshold_grant: Dictionary = restored_store.grant_match_xp(account_id, "match-test-2", 350)
	assert(bool(threshold_grant.get("awarded", false)))
	assert((threshold_grant.get("newly_unlocked", []) as Array).size() == 10)
	var threshold_account: Dictionary = threshold_grant.get("account", {})
	var threshold_inventory: Dictionary = threshold_account.get("inventory", {})
	assert((threshold_inventory.get("card_backs", []) as Array).size() == 2)
	assert((threshold_inventory.get("reactions", []) as Array).size() == 25)
	assert((threshold_inventory.get("gifts", []) as Array).size() == 13)
	assert((threshold_inventory.get("soundbar_folders", []) as Array).size() == 2)
	assert(int((threshold_account.get("next_reward", {}) as Dictionary).get("xp", 0)) == 1000)
	...
	assert(int((duplicate_xp_grant.get("account", {}) as Dictionary).get("xp", 0)) == 500)
	...
	assert(int(persisted_account.get("xp", 0)) == 500 and int(persisted_account.get("completed_matches", 0)) == 2)
	assert(((persisted_account.get("inventory", {}) as Dictionary).get("reactions", []) as Array).size() == 25)
